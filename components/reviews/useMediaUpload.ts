  ...
  async function startUpload(file: File) {
    ...
    try {
      ...
      const {
        ...
        setAttachments(prev => prev.map(attachment => attachment.id === id ? {...attachment, state: 'uploaded', url} : attachment))

    } catch (error) {
      console.error(error)
      setAttachments(prev => prev.map(attachment => attachment.id === id ? {...attachment, state: 'failed'} : attachment))
      toast({
        variant: 'destructive',
